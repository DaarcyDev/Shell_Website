...
-- Schema shell
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `shell` DEFAULT CHARACTER SET utf8mb4 ;
USE `shell` ;
...
CREATE TABLE IF NOT EXISTS `shell`.`admin` (
  ...
  PRIMARY KEY (`idadmin`),
  INDEX `fk_admin_Register1_idx` (`Register_idRegister` ASC),
  CONSTRAINT `fk_admin_Register1`
    FOREIGN KEY (`Register_idRegister`)
    REFERENCES `shell`.`register` (`idRegister`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `shell`.`about` (
  ...
  CONSTRAINT `fk_about_admin1`
    FOREIGN KEY (`admin_idadmin`)
    REFERENCES `shell`.`admin` (`idadmin`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `shell`.`discography` (
  ...
  `SpotifyLink` VARCHAR(150) NOT NULL,
  ...
  CONSTRAINT `fk_discography_admin1`
    FOREIGN KEY (`admin_idadmin`)
    REFERENCES `shell`.`admin` (`idadmin`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
